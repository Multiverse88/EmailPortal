...
function Inbox_() {
  ...
  return (
    <div className="app-shell h-[100dvh]">
                    ...
                    <div className="mt-8 pt-6 border-t border-slate-100">
                      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                        ...
                        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-50 border border-emerald-200/80 text-[11px] font-medium text-emerald-700">
                          ...
                          <span>Pemeriksaan Keamanan Ketat Aktif</span>
                        </div>
                      </div>
                        ...
                        {message.attachments.map((a) => (
                          ...
                          >
                            <button
                              ...
                              onClick={() => a.scanStatus !== 'quarantined' && setPreviewAttachment(a)}
                              disabled={a.scanStatus === 'quarantined'}
                              className="group flex min-w-0 flex-1 items-center gap-3 rounded-lg p-1 text-left focus-visible:outline-none disabled:cursor-not-allowed"
                              ...
                              <div className="min-w-0">
                                ...
                                <div className="mt-0.5 flex items-center gap-1.5 text-[10px] text-slate-400 flex-wrap">
                                  ...
                                  {a.scanStatus === 'quarantined' ? (
                                    <span className="inline-flex items-center gap-0.5 font-semibold text-amber-700 bg-amber-50 px-1.5 py-0.2 rounded border border-amber-200/80">
                                      <ShieldAlert className="h-3 w-3 text-amber-600" /> Menunggu Verifikasi IT
                                    </span>
                                  ) : (
                                    <>
                                      ...
                                      <span aria-hidden="true">•</span>
                                      <span className="inline-flex items-center gap-0.5 font-semibold text-emerald-700 bg-emerald-50 px-1.5 py-0.2 rounded border border-emerald-200/70">
                                        <ShieldCheck className="h-3 w-3 text-emerald-600" /> Keamanan Terverifikasi
                                      </span>
                                    </>
                                  )}
                                </div>
                            ...
                            <button
                              ...
                              disabled={downloadingId === a.id || a.scanStatus === 'quarantined'}
                              className="app-icon-button !size-8 shrink-0 disabled:opacity-50 disabled:cursor-not-allowed"
                              aria-label={`Unduh ${a.filename}`}
                              title={a.scanStatus === 'quarantined' ? 'Berkas dalam tinjauan keamanan tim IT' : 'Unduh berkas'}
                            >
                              {downloadingId === a.id ? (
      ...
      <LegalDisclaimerModal open={showDisclaimer} onAccept={handleAcceptDisclaimer} />
    </div>
  );
}
