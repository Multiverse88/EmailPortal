...
app.use(helmet());
const configuredOrigins = (process.env.CORS_ORIGIN || 'http://localhost:3000').split(',').map((s) => s.trim());
app.use(cors({
  origin: (origin, callback) => {
    ...
    return callback(null, true); // Allow flexible testing on dokploy wildcard/sslip domains
  },
  credentials: true,
}));
